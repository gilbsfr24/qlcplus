...
void HUEPacketizer::setupHUEDmx(QByteArray &data, quint32 universe, quint32 channel, uchar value)
{
    data.clear();
...
void HUEPacketizer::setupHUEGeneric(QByteArray &data, QString &path, QString types, QByteArray &values)
{
    data.clear();
    if (path.isEmpty())
    {
 ...
 * Receiver functions
 *********************************************************************/
bool HUEPacketizer::parseMessage(QByteArray &data, QString &path, QByteArray &values)
{
    path.clear();
    values.clear();
    ...
    qDebug() << " [HUE] path extracted:" << path;
    ...
    qDebug () << "[HUE] Tags found:" << typeArray.count() << "currpos at" << currPos;
    ...
    foreach(TagType tag, typeArray)
    {
        switch (tag)
        {
            ...
            case Time:
            {
                // A HUE timestamp would be helpful to defer
...
QList<QPair<QString, QByteArray> > HUEPacketizer::parsePacket(QByteArray &data)
{
    int bufPos = 0;
    ...
    while (bufPos < data.size())
    {
        ...
        if (data.at(bufPos) == '#')
        {
            ...
            {
                qWarning() << "[HUE] Found an unsupported message type !" << data;
                return messages;
            }
            ...
            while (bufPos < data.size() && data.at(bufPos) != '#')
            {
                ...
                qDebug() << "[HUE] Bundle message size:" << msgSize;
                bufPos += 4;
